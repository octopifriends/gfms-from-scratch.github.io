...
# Check for TerraTorch (optional for this demo)
try:
    import terratorch
    print(f"TerraTorch version: {terratorch.__version__}")
    TERRATORCH_AVAILABLE = True
except ImportError:
    print("TerraTorch not installed (optional for this demo)")
    print("To install: pip install terratorch")
...
def load_pretrained_model(
    ...
    """
    Load a pretrained TerraTorch model.
...
from geogfm.utils.terratorch_utils import load_pretrained_model, get_model_info

# For this demo, we'll work with a simpler backbone
# In practice, you would use models like 'prithvi_100m' or 'clay_v1'
try:
    # Try to load a simple model (adjust model name based on availability)
    model = load_pretrained_model(
        model_name='prithvi_eo_v1_100m',
        task='embedding',
        device=str(device)
    )
    ...
    print(f"Note: Full TerraTorch models require specific installation")
    print(f"Error: {e}")
    print("\nCreating a simple demonstration model instead...")
...
```{python}
from geogfm.utils.terratorch_utils import extract_features
...
```{python}
from geogfm.utils.terratorch_utils import visualize_embeddings
...
```{python}
from geogfm.training.simple_trainer import SimpleDataset, train_classifier, evaluate_model
