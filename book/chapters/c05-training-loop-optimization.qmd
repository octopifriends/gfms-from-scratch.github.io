...
```{python}
#| tangle: geogfm/training/__init__.py
#| header: "geogfm.training — package init (Week 5). Training utilities (optimizer, loop)."
# geogfm.training
```
...
```{python}
#| tangle: geogfm/training/optimizer.py
#| header: "geogfm.training.optimizer — Optimizer builder (Week 5): AdamW by default, Adam optional."
...
```{python}
#| tangle: geogfm/training/loop.py
#| header: "geogfm.training.loop — Minimal fit/evaluate loop (Week 5): logging and patch-based loss."
from __future__ import annotations
